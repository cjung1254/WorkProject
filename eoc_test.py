...
class Chapter8OpeningQuote(OpeningQuote, PiCreatureScene):
    ...
    def get_formula(self):
        ...
        result.set_color_by_tex("h", ORANGE, substring = False)
        result.set_color_by_tex("d\\theta", ORANGE)
...
class InCarRestrictedView(ShowSpeedometer):
    ...
    def construct(self):
        ...
        eight = TexMobject("10").move_to(time_label[1])
        self.play(Transform(
            time_label[1], eight,
...
class PlotVelocity(GraphScene):
    ...
    def get_v_graph_and_label(self):
        ...
        graph_label = TexMobject("v(t)", "=t(10-t)")
...
class AskAboutAntiderivative(PlotVelocity):
    ...
    def write_antiderivative(self):
        ...
        words.set_color_by_tex("t(10-t)", VELOCITY_COLOR)
...
class Antiderivative(PiCreatureScene):
    def construct(self):
        ...
        alt_functions = self.get_functions("???", "t(10-t)")
...
class AreaUnderVGraph(PlotVelocity):
    ...
    def show_rects(self):
        ...
        rects = self.get_riemann_rectangles(
            flat_graph, x_min = 0, x_max = 10, dx = 1.0
        )

        for new_rects in rect_list:
            new_rects.set_fill(opacity = 1.10)
            rects.align_submobjects(new_rects)
            for alt_rect in rects[::2]:
                alt_rect.set_fill(opacity = 1)
            self.play(Transform(
                rects, new_rects,
                run_time = 2,
                lag_ratio = 0.5
...
class ConstantForcePlot(PlotVelocity):
    CONFIG = {
        "x_max" : 50,
        "y_max" : 35,
        "x_axis_label" : "Distance (m)",
        "y_axis_label" : "Force (N)",
        "units_of_area_color" : GREEN_B,
        "x_tick_frequency" : 5,
        "x_labeled_nums" : None,
        "y_labeled_nums" : list(range(5, 35, 5)),
    }
    def construct(self):
        self.setup_axes()
    ...
    def draw_graph(self):
        ...
        self.play(ShowCreation(graph, rate_func=linear, run_time = 2))
        self.wait()
    ...
    def show_product(self):
        ...
        v_label.set_color(BLUE_B)
        t_label = top_brace.get_text(
            "50 meters"
        )
        t_label.set_color(YELLOW_E)

        s_label = TexMobject(r"750 \sqrt{3}",  "\\text{ Nm}")
        s_label.set_color_by_tex("10", VELOCITY_COLOR)
        s_label.set_color_by_tex("10", TIME_COLOR)
...
class PiecewiseConstantPlot(PlotVelocity):
    ...
    def write_integral_symbol(self):
        ...
        bounds = list(map(integral.get_part_by_tex, ["0", "10"]))
        ...
        self.wait()
        for bound, num in zip(bounds, [0, 10]):
    ...
    def get_ticks(self, rects):
        ticks = VGroup(*[
            ...
            for t in np.linspace(0, 10, len(rects)+1)
...
class FindAntiderivative(Antiderivative):
    ...
    def introduce(self):
        q_marks, rhs = functions = self.get_functions("???", "t(10-t)")
        expanded_rhs = TexMobject("10t - t^2")
...
class LowerBound(AreaIsDerivative):
    ...
    def write_antiderivative_difference(self):
        ...
        antideriv_diff_at_eight = self.get_antiderivative_difference("0", "10")
        at_eight = antideriv_diff_at_eight.left_part
        integral_at_eight = self.get_integral("0", "10")
        ...
        self.wait(2)
        self.change_area_bounds(0, 10, run_time = 2)
        self.play(
    ...
    def show_alternate_antiderivative_difference(self):
        ...
        for tex_mob in tex_mobs:
            ...
            tex_mob.set_color_by_tex("7", ORANGE)
...
class NegativeArea(GraphScene):
    ...
    def show_rectangle(self):
        ...
        rects = rect_list[0]
        rect = rects[len(rects)*self.sample_input//10]
...
class Vectors(Scene):
    def construct(self):
        ...
        vector2 = Vector([1, 1.73, 0])
        arc = Arc(
            angle_of_vector(3), 
            radius = vector1_length / 4.
        )
...
class DownRamp(Scene):
    ...
    def velocity_vector(self, parabola):
        ...
        vy = Vector(vector[-3]*UP, color = RED)
        v_mob.shift(p1)
        vx.shift(p1)
        vy.shift(vx.get_end())
        ...
        v_label.shift(p1 + RIGHT*vector[0]/4 + UP*vector[-3]/2)
        v_label.set_color(v_mob.get_color())
        vx_label = TexMobject("||\\vec{v}|| \\cos(\\theta)")
        vx_label.next_to(vx, DOWN)
